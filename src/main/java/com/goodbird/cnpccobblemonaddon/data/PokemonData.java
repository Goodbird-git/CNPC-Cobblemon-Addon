...
public class PokemonData {
    ...
    public CompoundTag serializeNBT(){
        ...
        for(HeldItemEntry item : this.heldItems){
            heldItems.add(item.serializeNBT());
        }
        tag.put("heldItems", heldItems);

        tag.put("dropItems", dropData.serializeNBT());
        return tag;
    }

    public PokemonData parseNBT(CompoundTag tag){
        species = new ResourceLocation(tag.getString("species"));
        ...
        for(int i=0; i<this.heldItems.length; i++){
            this.heldItems[i] = new HeldItemEntry().parseNBT(heldItems.getCompound(i));
        }

        dropData.parseNBT(tag.getCompound("dropItems"));
        return this;
    }
    ...
    public static class HeldItemEntry {
        ...
        public CompoundTag serializeNBT(){
            CompoundTag tag = new CompoundTag();
            tag.put("stack", heldStackEntry.serializeNBT());
            tag.putDouble("chance", chance);
            return tag;
        }

        public HeldItemEntry parseNBT(CompoundTag tag){
            heldStackEntry = ItemStack.of(tag.getCompound("stack"));
