...
public class PokemonDropData {
    ...
    public CompoundTag serializeNBT(){
        ...
        for(DropItemEntry item : this.dropItems){
            dropItems.add(item.serializeNBT());
        }
        tag.put("dropItems", dropItems);
        return tag;
    }

    public PokemonDropData parseNBT(CompoundTag tag){
        ...
        for(int i=0; i<this.dropItems.length; i++){
            this.dropItems[i] = new DropItemEntry().parseNBT(dropItems.getCompound(i));
        }
        return this;
    }
    ...
    public static class DropItemEntry {
        ...
        public DropEntry getDropEntry(){
            ...
            entry.setItem(ForgeRegistries.ITEMS.getKey(dropStackEntry.getItem()));
            ...
            if(dropStackEntry.hasTag()){
                entry.setNbt(dropStackEntry.getTag());
            }
            return dropEntry;
        }
        ...
        public CompoundTag serializeNBT(){
            CompoundTag tag = new CompoundTag();
            tag.put("stack", dropStackEntry.serializeNBT());
        ...
        public PokemonDropData.DropItemEntry parseNBT(CompoundTag tag){
            dropStackEntry = ItemStack.of(tag.getCompound("stack"));
